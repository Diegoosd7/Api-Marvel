...
export default function ComicSelect() {
  ...
  const params = useParams(); // (UseParams es para rutas)
  const idcomic = parseInt(params.idcomic, 10); // Le hacemos un parseINT para guardarlo como número
  ...
  useEffect(() => {
    ...
    async function fetchInfoComic(idcomic) {
      const comic = await infoComic(idcomic); // Le pasamos, a la función creada en services, el id del Comic que pinchamos
      console.log(comic);
          ...
          <div className="infoComic">
            <div className="informacion">
              ...
              <div>
                ...
                {' '}
                {/* Precio del comic */}
                <p className="informacion__div__p">{comic.description}</p>
                <br />
                {' '}
                {/* Descripcion del comic */}
                <p className="informacion__div__p--titulo">
                  SERIE:
                  <span className="blanco">{comic.series.name}</span>
                </p>
                <br />
                {' '}
                {/* Serie a la que pertenece el comic */}
                <p className="informacion__div__p--titulo">
                  PÁGINAS:
                  <span className="blanco">{comic.pageCount}</span>
                </p>
                <br />
                {' '}
                {/* Páginas que tiene el comic */}
                <p className="informacion__div__p--titulo">PRECIO:</p>
                <br />
                <Precio precios={comic.prices} />
                {' '}
                {/* Le pasamos un array con los precios del comic */}
              </div>
            </div>
            ...
            {' '}
            {/* Le pasamos un array con los enlace de información del comic */}
          </div>
        ))
      }
    </div>
